        ...
        <>
            ...
            <div className="flex flex-col w-full">
                <div className="mt-16 w-full mr-5 p-5 bg-secondary rounded-md">
                    ...
                    {categoridata?.length > 0 ? categoridata?.map((category, index) => {
                        return (
                            <div key={index} className="flex items-center justify-between bg-primary p-5 rounded-md">
                                <div className="flex items-center gap-2">
                                    <div>
                                        <Image src={category?.image} width={72} height={72} alt="logo" />
                                    </div>
                                    <div>
                                        <h1 className="text-white text-xl font-semibold">{category?.name}</h1>
                                        <p className="text-white/70 text-base">{category?.date}</p>
                                    </div>
                                </div>
                                <div className="flex items-center gap-2">
                                    <Button>Edit</Button>
                                    <Button variant="danger" onClick={() => showDeleteModalHandler(category?.id)}>Delete</Button>
                                </div>
                            </div>
                        );
                    }) : (
                        ...
                        <div className="p-5 py-20">
                            <div className="text-center flex flex-col justify-center items-center">
                                ...
                                <p className="text-[13px] text-slate-400">
                                    Once you delete this category, it will be gone forever. Please confirm to proceed.
                                </p>
